...
public class Dashboard extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		int childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(MeasureSpec
				.getSize(widthMeasureSpec), MeasureSpec.AT_MOST);
		int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(MeasureSpec
				.getSize(widthMeasureSpec), MeasureSpec.AT_MOST);
		...
		for (int i = 0; i < count; i++) {
			...
			mMaxChildHeight = Math.max(mMaxChildHeight, child
					.getMeasuredHeight());
		}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		for (int i = 0; i < count; i++) {
			...
			int w = (hSpace == 0 && col == cols - 1) ? r : (left + width);
			int h = (vSpace == 0 && row == rows - 1) ? b : (top + height);
//			Log.i("dashboard","child.layout["+i+"] left: " + left + ", top: " + top + ", w: " + w + ", h: " + h);
			child.layout(left, top, w, h);
			++visibleIndex;
		}
	}
}